...
class BertForMultiOutputClassification(BertPreTrainedModel):
    """BERT model for multi-output classification"""
...
class BertForMultiPairClassification(BertPreTrainedModel):
    """
    BERT model for CPA (Column Pair Annotation) tasks.
    It uses BertMultiPairPooler to capture the interaction between
    the global context ([CLS]) and local column headers.
    """
    ...
    def __init__(self, config: transformers.BertConfig) -> None:
        ...
        self.num_labels = config.num_labels
        self.bert = BertModelMultiOutput(config)
        self.bert.pooler = BertMultiPairPooler(config)    
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, self.config.num_labels)
    ...
    def forward(
        ...
        raw_pooled_output = outputs[1] 
        pooled_output = self.dropout(raw_pooled_output)
        logits = self.classifier(pooled_output)
        output_tuple = (logits,) + outputs[2:] # hidden_states, attentions
        ...
        if return_features:
            return (logits, raw_pooled_output)
