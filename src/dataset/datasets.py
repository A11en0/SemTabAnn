...
class SatoCVColwiseDataset(Dataset):
    """SATO column-wise dataset for cross-validation"""
    ...
    def __init__(
        ...
        assert split in ["train", "valid", "test"], "split must be train or test"
        ...
        row_list = []
        groupby_df = df.groupby("table_id")
        for i, (index, group_df) in enumerate(tqdm.tqdm(groupby_df, total=num_tables)):
...
class SatoCVTablewiseDataset(Dataset):
    """SATO table-wise dataset for cross-validation"""
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        num_cols = row["num_col"]
        # Create column indices for all columns in this table (0, 1, 2, ..., num_cols-1)
...
class TURLColTypeColwiseDataset(Dataset):
    ...
    def __init__(self,
        ...
        row_list = []
        groupby_df = self.df.groupby("table_id")
        for i, (index, group_df) in enumerate(tqdm.tqdm(groupby_df, total=num_tables)):
            if i >= num_train:
                break
...
class TURLColTypeTablewiseDataset(Dataset):
    """TURL column type prediction table-wise"""
    ...
    def __init__(self,
        ...
        if os.path.exists(f"data/{folder}serialization/cta_{split}_tablewise.pkl"):
            ...
            data_list = []
            groupby_df = self.df.groupby("table_id")
            for i, (index, group_df) in tqdm.tqdm(enumerate(groupby_df), total=num_tables):
...
class TURLRelExtTablewiseDataset(Dataset):
    """TURL column relation prediction table-wise"""
